...
def start_dailychallenge(request):
    ...
    if created or player_stats.questions_answered < 10: 
        return render(request, 'loadfail.html', context={"reason": "You need to answer at least 10 questions in normal mode first."})
    
    today = date.today()
    if GameSession.objects.filter(user=player, start_time__date=today, is_dailychallenge=True).exists():
        return render(request, 'loadfail.html', context={"reason": "You have already attempted today's daily challenge."})
